...
async def create_image_rating(
    ...
    """
    image = await Image.get_image_by_id(image_id, db)
    if not image:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Image not found")
    if image.user_id == current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Cannot rate own image")
    rating = await ImageRatingService.create(rating_data.rating, rating_data.image_id, current_user.id, db)
    return rating


@router.put("/{image_id}/{rating_id}", response_model=ImageRatingUpdate)
async def update_image_rating(
    ...
    rating = await rating.update(db, **rating_data.dict())
    return rating
...
async def get_all_image_ratings(image_id: int, db=Depends(get_db)):
    """
    ...
    """
    ratings = await ImageRatingService.get_all_ratings(image_id, db)
    return ratings
