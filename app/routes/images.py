...
@router.get("/", response_model=list[ImagePublic], description="Get all images" + docs.GET_IMAGES,
...
@router.get("/{image_id}", response_model=ImagePublic, description=docs.GET_IMAGE)
async def get_image(
        image_id: int,
        db: AsyncSession = Depends(get_db),
        current_user: User = Depends(get_current_active_user)
...
@router.patch("/", response_model=ImagePublic, dependencies=[Depends(RateLimiter(times=10, seconds=60))],
              description=docs.UPDATE_IMAGE_DATA)
